...
에이전트는 실행 환경에서 실제로 쓸 수 있는 모델을 확인합니다. 작업을 할 수 있는 모델 중 비용이 적게 드는 것을 고릅니다. 바로 아래 모델뿐 아니라 훨씬 작은 모델도 선택할 수 있습니다. 필요한 도구, 입력 크기, 난이도, 실패 영향을 살펴봅니다. 사용자가 모델을 지정하면 그 선택을 따릅니다.

단순 검사는 기존 도구로 실행합니다. 테스트가 실패했다고 바로 큰 모델로 바꾸지 않습니다. 피드백을 받아도 판단 오류가 반복되면 남은 예산 안에서 모델을 바꿀 수 있습니다. 리뷰와 승인 기준은 그대로 유지합니다.

kit은 [모델 선택 규칙](https://github.com/syjkim0125/ai-workflow-kit/blob/main/skills/workflow/references/model-selection.md)을 제공합니다. 실제 변경은 실행 환경이 지원해야 합니다. 지원하지 않으면 허용된 현재 모델이나 기본 모델을 유지하고 한계를 알립니다. 새 모델 서비스나 Jev를 추가하지 않습니다. 총비용이 줄었는지는 측정해야 합니다.

## Agent Office에서 사용하기

**kit는 맡은 일을 끝내는 절차를 관리하고, Office는 여러 사람의 일이 하나의 목표로 이어지도록 관리합니다.**

Office는 PM, 팀장, Developer, Reviewer에게 일을 배정합니다. 각 에이전트는 kit으로 자기 역할을 수행합니다. Office 없이도 같은 역할별 절차를 사용할 수 있습니다.
...
| **Office** | 공통 완료 기준, 역할 배정, 에이전트 실행, 메시지, 결과 통합, 예산, 취소, 사용자 승인 |
| **에이전트** | 배정된 작업, 결과, 증거, 질문 |

kit 실행 파일은 개별 작업의 진행 상태를 관리합니다. Office는 공통 목표와 협업을 관리합니다. Office가 kit의 진행 상태를 표시할 수 있지만, 같은 상태를 따로 변경하면 안 됩니다. PM과 Developer의 완료 기준이 다르면 Office가 조율합니다. 에이전트가 자기 기준으로 요구사항을 바꾸면 안 됩니다.
...
별도 역할을 맡으면 `init-role`로 시작합니다. PM은 요구사항을 정리하고, 팀장은 조사와 계획을 합니다. Developer는 구현과 자체 검사를 하고, Reviewer는 검토 결과를 반환합니다. 역할이 끝나면 `role-complete`를 반환합니다. 전체 프로젝트 완료는 아닙니다.

`question`은 질문을 기다립니다. `answer`는 같은 시도를 새 토큰으로 재개합니다. `feedback`은 지적받은 작업과 후속 작업을 다시 열어 수정하게 합니다. 답변은 승인이 아닙니다. [역할별 사용법과 JSON 예시](https://github.com/syjkim0125/ai-workflow-kit/blob/main/skills/workflow/references/role-graphs.md)를 참고하세요.
