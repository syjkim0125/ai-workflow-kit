...
- Questions are not failed tests. Send a question with task/attempt, target revision and question identity to the host. Use its existing pause/reply facilities; an answer is not automatically approval. The current local Graph CLI has no dedicated question/reply transition: report that boundary rather than inventing a `resume` command or silently exhausting retry attempts.
